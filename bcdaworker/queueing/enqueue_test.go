...
func TestQueEnqueuer(t *testing.T) {
	...
	sb.Where(sb.Equal("args ->> 'ID'", jobArgs.ID), sb.Equal("args ->> 'ACOID'", jobArgs.ACOID),
	...
	delete.Where(delete.Equal("args ->> 'ID'", jobArgs.ID), delete.Equal("args ->> 'ACOID'", jobArgs.ACOID),
