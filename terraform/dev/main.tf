...
# FARGATE FIXUP: remove me
data "aws_security_group" "zscaler_public" {
  name = local.zscaler_public_name
  filter {
    name   = "vpc-id"
    values = [local.vpc_id]
  }
  ...
  policy = <<EOF
{
    ...
    "Statement": [
        ...
        },
        {
            ...
            "Resource": [
              "arn:aws:s3:::${local.aco_credentials_bucket}/*"
            ]
        },
        {
            "Effect": "Allow",
        ...
        },
        {
          ...
          "Resource": "arn:aws:ssm:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:parameter/bcda/${var.env}/*"
        },
        {
          "Effect": "Allow",
...
resource "aws_lb" "alb_01" {
  ...
  security_groups = flatten([aws_security_group.sg_alb_01.id, data.aws_security_group.zscaler_public.id])
...
resource "aws_lb" "ssas_alb" {
  ...
  security_groups = flatten([
    ...
    data.aws_security_group.zscaler_public.id,
...
resource "aws_security_group_rule" "app_sg_ingress_http_01" {
  type                     = "ingress"
...
resource "aws_security_group" "worker_sg" {
  ...
  description = "bcda worker security group"
  vpc_id      = local.vpc_id
...
module "cloudwatch_alarms_elb_http" {
  ...
  alarm_backend_4xx_enable      = false
  alarm_backend_5xx_enable      = false
  alarm_elb_5xx_enable          = false
  alarm_elb_high_latency_enable = false
...
module "ecs_api" {
  ...
  image                 = "${module.ecr_api.this.repository_url}:${var.api_image_tag}"
  cpu                   = local.ecs_task_def_cpu_api
  memory                = local.ecs_task_def_mem_api
  desired_count         = local.api_desired_min
  port_mappings         = [{ containerPort = local.app_port }, { containerPort = local.app_http_port }]
...
module "ecs_ssas" {
  ...
  platform              = module.platform
  cluster_arn           = module.ecs_cluster.this.arn
  image                 = "${module.ecr_ssas.this.repository_url}:${var.ssas_image_tag}"
...
module "ecs_worker" {
  ...
  platform              = module.platform
  cluster_arn           = module.ecs_cluster.this.arn
  image                 = "${module.ecr_worker.this.repository_url}:${var.worker_image_tag}"
...
resource "aws_appautoscaling_policy" "ecs_worker_policy" {
  ...
  step_scaling_policy_configuration {
    ...
    step_adjustment {
      scaling_adjustment          = local.worker_desired_min + 1
