# This module is for defining ELB - HTTP CloudWatch alarms

resource "aws_cloudwatch_metric_alarm" "healthy_hosts" {
  count               = var.alarm_elb_no_backend_enable ? 1 : 0
  ...
  treat_missing_data = var.alarm_elb_no_backend_treat_missing_data
  alarm_actions      = [var.cloudwatch_notification_arn]
  ok_actions         = [var.cloudwatch_notification_arn]
}

resource "aws_cloudwatch_metric_alarm" "high_latency" {
  count               = var.alarm_elb_high_latency_enable ? 1 : 0
  ...
  treat_missing_data = var.alarm_elb_high_latency_treat_missing_data
  alarm_actions      = [var.cloudwatch_notification_arn]
  ok_actions         = [var.cloudwatch_notification_arn]
}

resource "aws_cloudwatch_metric_alarm" "spillover_count" {
  count               = var.alarm_elb_spillover_count_enable ? 1 : 0
...
resource "aws_cloudwatch_metric_alarm" "surge_queue_exceeded" {
  count               = var.alarm_elb_surge_queue_length_enable ? 1 : 0
...
resource "aws_cloudwatch_metric_alarm" "httpcode_backend_4xx" {
  count               = var.alarm_backend_4xx_enable ? 1 : 0
...
resource "aws_cloudwatch_metric_alarm" "httpcode_backend_5xx" {
  count               = var.alarm_backend_5xx_enable ? 1 : 0
...
resource "aws_cloudwatch_metric_alarm" "httpcode_elb_5xx" {
  count               = var.alarm_elb_5xx_enable ? 1 : 0
