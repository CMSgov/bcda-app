...
unit-test-db:
	...
	docker run --rm -v /db/migrations:/migrations --network bcda-app-net migrate/migrate -path=/migrations/bcda_queue/ -database 'postgres://postgres:toor@queue:5432/bcda?sslmode=disable&x-migrations-table=schema_migrations_bcda_queue' up
...
load-fixtures-ssas:
	docker run --rm --network prototype-net migrate/migrate:v4.15.0-beta.3 -source='github://CMSgov/bcda-ssas-app/db/migrations#master' -database 'postgres://postgres:toor@db:5432/bcda?sslmode=disable' up
