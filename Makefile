...
postman:
	...
	$(eval CLIENT_TEMP := $(shell docker-compose run --rm api bcda reset-client-credentials --cms-id $(ACO_CMS_ID) | tail -n2))
...
load-fixtures:
	...
	$(eval ACO_CMS_ID = A9994)
	docker-compose run --rm api bcda generate-synthetic-alr-data --cms-id=$(ACO_CMS_ID) --alr-template-file ./alr/gen/testdata/PY21ALRTemplatePrelimProspTable1.csv
...
load-synthetic-cclf-data:
	...
	for acoSize in $(ACO_SIZES) ; do \
		docker-compose run --rm api bcda import-synthetic-cclf-package --acoSize=$$acoSize --environment=test ; \
	done
	...
	for acoSize in $(ACO_SIZES)  ; do \
		docker-compose run --rm api bcda import-synthetic-cclf-package --acoSize=$$acoSize --environment=test-new-beneficiaries ; \
	done

	for acoSize in $(ACO_SIZES)  ; do \
		docker-compose run --rm api bcda import-synthetic-cclf-package --acoSize=$$acoSize --environment=test --fileType=runout ; \
	done

load-synthetic-suppression-data:
	docker-compose run --rm api bcda import-suppression-directory --directory=../shared_files/synthetic1800MedicareFiles
...
credentials:
	...
	docker-compose run --rm api bcda reset-client-credentials --cms-id $(ACO_CMS_ID) | tail -n2
