...
refresh_attribution () {
    ...
    for aco in ${PREV_PY_ACOS[@]}; do
        echo "Updating: $aco with attribution for previous year"
            ...
            psql -t $conn -c "UPDATE cclf_files SET timestamp = NOW()::date + timestamp::time, performance_year = (EXTRACT(YEAR FROM NOW())::int - 1) % 100 WHERE id IN (SELECT id FROM cclf_files WHERE name LIKE 'T.%' AND name LIKE '%ZC8%' AND aco_cms_id = '${aco}' ORDER BY timestamp DESC LIMIT 2);"
    done
}
