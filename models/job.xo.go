...
func (j *Job) Insert(db XODB) error {
	...
	const sqlstr = `INSERT INTO public.jobs (` +
		`aco_id, user_id, request_url, status, created_at` +
		`) VALUES (` +
		`$1, $2, $3, $4, $5` +
		`) RETURNING id`

	// run query
	XOLog(sqlstr, j.AcoID, j.UserID, j.RequestURL, j.Status, j.CreatedAt)
	err = db.QueryRow(sqlstr, j.AcoID, j.UserID, j.RequestURL, j.Status, j.CreatedAt).Scan(&j.ID)
	if err != nil {
		return err
	}
...
func (j *Job) Update(db XODB) error {
	...
	// run query
	XOLog(sqlstr, j.AcoID, j.UserID, j.RequestURL, j.Status, j.CreatedAt, j.ID)
	_, err = db.Exec(sqlstr, j.AcoID, j.UserID, j.RequestURL, j.Status, j.CreatedAt, j.ID)
	return err
}
...
func (j *Job) Upsert(db XODB) error {
	...
	// sql query
	const sqlstr = `INSERT INTO public.jobs (` +
		`id, aco_id, user_id, request_url, status, created_at` +
		`) VALUES (` +
		`$1, $2, $3, $4, $5, $6` +
		`) ON CONFLICT (id) DO UPDATE SET (` +
		`id, aco_id, user_id, request_url, status, created_at` +
		`) = (` +
		`EXCLUDED.id, EXCLUDED.aco_id, EXCLUDED.user_id, EXCLUDED.request_url, EXCLUDED.status, EXCLUDED.created_at` +
		`)`

	// run query
	XOLog(sqlstr, j.ID, j.AcoID, j.UserID, j.RequestURL, j.Status, j.CreatedAt)
	_, err = db.Exec(sqlstr, j.ID, j.AcoID, j.UserID, j.RequestURL, j.Status, j.CreatedAt)
	if err != nil {
		return err
	}
...
func JobByID(db XODB, id int) (*Job, error) {
	...
	err = db.QueryRow(sqlstr, id).Scan(&j.ID, &j.AcoID, &j.UserID, &j.RequestURL, &j.Status, &j.CreatedAt)
	if err != nil {
		return nil, err
	}
