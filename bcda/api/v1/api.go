...
func ServeData(w http.ResponseWriter, r *http.Request) {
	...
	encoded, err := isGzipEncoded(rootDir, fileName)
	if err != nil {
...
func isGzipEncoded(root *os.Root, filePath string) (encoded bool, err error) {
	file, err := root.Open(filePath) // #nosec G703
	if err != nil {
		return false, err
	}
	defer file.Close() // #nosec G307
...
func (a ApiV1) HealthCheck(w http.ResponseWriter, r *http.Request) {
	...
	introspectStatus, introspectOK := a.healthChecker.IsSsasIntrospectOK()

	m["database"] = dbStatus
	m["ssas"] = ssasStatus
	m["ssas_introspect"] = introspectStatus

	if !dbOK || !ssasOK || !introspectOK {
