...
func TestValidateTypeFilterPACEligibility(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			cmsID:       "PAC0000",
			typeFilter:  makeTypeFilterParam([][]string{{"_tag", (constants.BBCodeSystemURL + "System-Type|SharedSystem")}}),
			acoConfig:   acoWithPAC,
			shouldFail:  false,
...
func TestExtractTagCodeFromValue(t *testing.T) {
	tests := []struct {
		...
		{"urlFormatSystemType", constants.BBCodeSystemURL + "System-Type|SharedSystem", []string{"SharedSystem"}},
		{"urlFormatFinalAction", constants.BBCodeSystemURL + "Final-Action|FinalAction", []string{"FinalAction"}},
		...
		{"commaSeparatedUrl", constants.BBCodeSystemURL + "System-Type|SharedSystem," + constants.BBCodeSystemURL + "Final-Action|FinalAction", []string{"SharedSystem", "FinalAction"}},
	}
...
func TestOmitSharedSystemByDefault(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			cmsID:        "NOPAC0000",
			typeFilter:   makeTypeFilterParam([][]string{{"_tag", constants.BBCodeSystemURL + "System-Type|NationalClaimsHistory"}}),
			acoConfig:    acoWithoutPAC,
			expectedTags: []string{constants.BBCodeSystemURL + "System-Type|NationalClaimsHistory"},
			description:  "Non-PAC ACO with existing NCH filter should keep same filter",
		},
		{
			name:         "NonPACWithDDPSFilter",
			cmsID:        "NOPAC0000",
			typeFilter:   makeTypeFilterParam([][]string{{"_tag", constants.BBCodeSystemURL + "System-Type|DDPS"}}),
			acoConfig:    acoWithoutPAC,
			expectedTags: []string{constants.BBCodeSystemURL + "System-Type|DDPS"},
		...
		},
		{
			...
			typeFilter:   makeTypeFilterParam([][]string{{"_tag", constants.BBCodeSystemURL + "System-Type|SharedSystem"}}),
			acoConfig:    acoWithPAC,
			expectedTags: []string{constants.BBCodeSystemURL + "System-Type|SharedSystem"},
...
func TestEnsureSharedSystemOmittedForNonPACWithDefaultEOB(t *testing.T) {
	...
	expectedNCHTag := constants.BBCodeSystemURL + "System-Type|NationalClaimsHistory," + constants.BBCodeSystemURL + "System-Type|DDPS"
