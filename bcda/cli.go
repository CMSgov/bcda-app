...
func importCCLF0(fileMetadata *cclfFileMetadata) (map[string]cclfFileValidator, error) {
	if fileMetadata == nil {
		fmt.Print("file CCLF0 not found")
	...
	if err != nil {
		fmt.Printf("could not read CCLF0 archive %s", fileMetadata.name)
	...
	for i, f := range r.File {
		fmt.Printf("Reading file #%d from archive %s", i, fileMetadata.name)
		log.Infof("Reading file #%d from archive %s", i, fileMetadata.name)
		rc, err := f.Open()
		if err != nil {
			fmt.Printf("could not read file %s in CCLF0 archive %s", f.Name, fileMetadata.name)
		...
		for sc.Scan() {
			...
			if len(bytes.TrimSpace(b)) > 0 {
				...
				if filetype == "CCLF8" || filetype == "CCLF9" {
					...
					if err != nil {
						fmt.Printf("Failed to parse %s record count from CCLF0 file", filetype)
					...
					if err != nil {
						fmt.Printf("Failed to parse %s record length from CCLF0 file", filetype)
	...
	if _, ok := validator["CCLF8"]; !ok {
		fmt.Printf("failed to parse CCLF8 from CCLF0 file: %v", fileMetadata)
		err := fmt.Errorf("failed to parse CCLF8 from CCLF0 file: %v", fileMetadata)
		log.Error(err)
		return nil, err
	}
	if _, ok := validator["CCLF9"]; !ok {
		fmt.Printf("failed to parse CCLF9 from CCLF0 file: %v", fileMetadata)
...
func importCCLF8(fileMetadata *cclfFileMetadata) error {
	err := importCCLF(fileMetadata, func(fileID uint, b []byte, db *gorm.DB) error {
		...
		if err != nil {
			fmt.Print("could not create CCLF8 beneficiary record")
...
func importCCLF9(fileMetadata *cclfFileMetadata) error {
	err := importCCLF(fileMetadata, func(fileID uint, b []byte, db *gorm.DB) error {
		...
		if err != nil {
			fmt.Print("could not create CCLF9 cross reference record")
...
func importCCLF(fileMetadata *cclfFileMetadata, importFunc func(uint, []byte, *gorm.DB) error) error {
	...
	r, err := zip.OpenReader(filepath.Clean(fileMetadata.filePath))
	if err != nil {
		fmt.Printf("could not read CCLF%d archive %s", fileMetadata.cclfNum, fileMetadata.name)
	...
	if len(r.File) < 1 {
		fmt.Printf("no files found in CCLF%d archive %s", fileMetadata.cclfNum, fileMetadata.name)
	...
	if err != nil {
		fmt.Printf("could not create CCLF%d file record", fileMetadata.cclfNum)
	...
	for i, f := range r.File {
		...
		rc, err := f.Open()
		if err != nil {
			fmt.Printf("could not read file %s in CCLF%d archive %s", f.Name, fileMetadata.cclfNum, fileMetadata.name)
...
func getCCLFFileMetadata(filePath string) (cclfFileMetadata, error) {
	...
	if len(filenameMatches) < 5 {
		fmt.Printf("invalid filename for file: %s", filePath)
	...
	if err != nil {
		fmt.Printf("failed to parse cclf num from file: %s", filePath)
	...
	if err != nil {
		fmt.Printf("failed to parse performance year from file: %s", filePath)
	...
	if err != nil || t.IsZero() {
		fmt.Printf("failed to parse date '%s' from file: %s", filenameDate, filePath)
...
func importCCLFDirectory(filePath string) (success, failure, skipped int, err error) {
	...
	for _, cclflist := range cclfmap {
		...
		if err != nil {
			fmt.Printf("Failed to import CCLF0 file: %v, Skipping CCLF8 file: %v and CCLF9 file: %v ", cclf0, cclf8, cclf9)
		...
		if err != nil {
			fmt.Printf("Failed to validate CCLF8 file: %v", cclf8)
			log.Errorf("Failed to validate CCLF8 file: %v", cclf8)
			failure++
		} else {
			if err = importCCLF8(cclf8); err != nil {
				fmt.Printf("Failed to import CCLF8 file: %v ", cclf8)
		...
		if err != nil {
			fmt.Printf("Failed to validate CCLF9 file: %v", cclf9)
			log.Errorf("Failed to validate CCLF9 file: %v", cclf9)
			failure++
		} else {
			if err = importCCLF9(cclf9); err != nil {
				fmt.Printf("Failed to import CCLF9 file: %v ", cclf9)
...
func sortCCLFFiles(cclfmap *map[string][]*cclfFileMetadata, skipped *int) filepath.WalkFunc {
	return func(path string, info os.FileInfo, err error) error {
		if err != nil {
			fmt.Printf("Error in sorting cclf file: %v,", info.Name())
		...
		if err != nil {
			...
			fmt.Printf("unknown file found: %v", metadata)
			...
			*skipped = *skipped + 1
			...
			if err != nil {
				fmt.Printf("error moving unknown file: %v to pending deletion dir", metadata)
...
func validate(fileMetadata *cclfFileMetadata, cclfFileValidator map[string]cclfFileValidator) error {
	...
	if fileMetadata.cclfNum == 8 {
		...
		fmt.Printf("unknown file type when validating file: %v,", fileMetadata)
	...
	r, err := zip.OpenReader(filepath.Clean(fileMetadata.filePath))
	if err != nil {
		fmt.Printf("could not read archive %s", fileMetadata.name)
	...
	for i, f := range r.File {
		...
		rc, err := f.Open()
		if err != nil {
			fmt.Printf("could not read file %s in archive %s", f.Name, fileMetadata.name)
		...
		for sc.Scan() {
			...
			if bytelength > 0 && bytelength <= validator.maxRecordLength {
				...
				if count > validator.totalRecordCount {
					fmt.Printf("maximum record count reached for file %s, Expected record count: %d, Actual record count: %d ", key, validator.totalRecordCount, count)
				...
				fmt.Printf("incorrect record length for file %s, Expected record length: %d, Actual record length: %d", key, validator.maxRecordLength, bytelength)
...
func deleteDirectoryContents(dirToDelete string) (filesDeleted int, err error) {
	fmt.Printf("preparing to delete directory '%v'", dirToDelete)
	log.Infof("preparing to delete directory '%v'", dirToDelete)
	f, err := os.Open(filepath.Clean(dirToDelete))
	if err != nil {
		fmt.Printf("could not open dir: %s", dirToDelete)
	...
	if err != nil {
		fmt.Printf("error reading files from dir: %s", f.Name())
	...
	err = f.Close()
	if err != nil {
		fmt.Printf("error closing dir: %s", f.Name())
	...
	for _, file := range files {
		...
		if err != nil {
			fmt.Printf("error deleting file: %s from dir: %s", file.Name(), dirToDelete)
	...
	fmt.Printf("Successfully deleted all files from dir: %s", dirToDelete)
...
func cleanupCCLF(cclfmap map[string][]*cclfFileMetadata) {
	for _, cclflist := range cclfmap {
		for _, cclf := range cclflist {
			fmt.Printf("Cleaning up file %v", cclf)
			...
			if !cclf.imported {
				...
				if int(elapsed) > deleteThresholdHr {
					...
					if err != nil {
						fmt.Printf("file: %v failed to cleanup properly", cclf)
						log.Errorf("file: %v failed to cleanup properly", cclf)
					} else {
						fmt.Printf("file: %v never ingested, moved to the pending deletion dir", cclf)
				...
				if err != nil {
					...
					fmt.Printf("file: %v successfully ingested, moved to the pending deletion dir", cclf)
