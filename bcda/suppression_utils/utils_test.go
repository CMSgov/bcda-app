...
type SuppressionUtilsTestSuite struct {
	suite.Suite
	basePath string
	cleanup  func()
}

func (s *SuppressionUtilsTestSuite) SetupTest() {
	s.basePath, s.cleanup = testUtils.CopyToTemporaryDirectory(s.T(), "../../shared_files/")
}

func (s *SuppressionUtilsTestSuite) TestParseMetadata() {
...
func (s *SuppressionUtilsTestSuite) TestParseMetadata_InvalidFilename() {
	assert := assert.New(s.T())
