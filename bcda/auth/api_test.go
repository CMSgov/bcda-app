...
func (s *AuthAPITestSuite) TestGetAuthTokenErrorSwitchCases() {
	...
	tests := []struct {
		...
		{"Token Rate Limit Too Many Requests Error Return 429", &customErrors.SSASErrorTooManyRequests{Err: errors.New(errorHappened), Msg: errMsg}, 429, "300", "cms_test exceeded the rate limit for /auth/token. Access tokens last 20 minutes; please cache and reuse them."},
