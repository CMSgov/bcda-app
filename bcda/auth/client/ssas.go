...
func NewSSASClient() (*SSASClient, error) {
	...
	if timeout, err = strconv.Atoi(conf.GetEnv("SSAS_TIMEOUT_MS")); err != nil {
		log.SSAS.Warn(errors.Wrap(err, "Could not get SSAS timeout from environment variable; using default value of 20000."))
		timeout = 20000
	}
...
func (c *SSASClient) CreateGroup(groupId, name, acoCMSID string) ([]byte, error) {
	b := fmt.Sprintf(`{"group_id": "%s", "name": "%s", "scopes": ["bcda-api"], "xdata": "{\"cms_ids\": [\"%s\"]}"}`, groupId, name, acoCMSID)
	req, err := http.NewRequest("POST", fmt.Sprintf("%s/group", c.baseURL), strings.NewReader(b))
	if err != nil {
		return nil, errors.Wrap(err, fmt.Sprintf("invalid input for new group_id %s", groupId))
	}
...
func (c *SSASClient) DeleteCredentials(systemID string) error {
	...
	if resp.StatusCode != http.StatusOK {
		return fmt.Errorf("%v; %v", constants.DeleteCredentialsErr, resp.StatusCode)
	}
