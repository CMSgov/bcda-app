...
func init() {
	...
	filePath := configuration.GetEnv("BCDA_SSAS_LOG")
...
func NewSSASClient() (*SSASClient, error) {
	...
	if configuration.GetEnv("SSAS_USE_TLS") == "true" {
	...
	var timeout int
	if timeout, err = strconv.Atoi(configuration.GetEnv("SSAS_TIMEOUT_MS")); err != nil {
		ssasLogger.Info("Could not get SSAS timeout from environment variable; using default value of 500.")
		timeout = 500
	}

	ssasURL := configuration.GetEnv("SSAS_URL")
	if ssasURL == "" {
...
func tlsTransport() (*http.Transport, error) {
	caFile := configuration.GetEnv("BCDA_CA_FILE")
...
func (c *SSASClient) GetToken(credentials Credentials) ([]byte, error) {
	public := configuration.GetEnv("SSAS_PUBLIC_URL")
...
func (c *SSASClient) VerifyPublicToken(tokenString string) ([]byte, error) {
	...
	clientID := configuration.GetEnv("BCDA_SSAS_CLIENT_ID")
	secret := configuration.GetEnv("BCDA_SSAS_SECRET")
	if clientID == "" || secret == "" {
