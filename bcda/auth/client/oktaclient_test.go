...
func (s *OTestSuite) TestConfig() {
	...
	originalOktaToken := os.Getenv("OKTA_API_KEY")

	os.Unsetenv("OKTA_CLIENT_ORGURL")
	os.Unsetenv("OKTA_OAUTH_SERVER_ID")
	os.Unsetenv("OKTA_API_KEY")

	err := config()
	require.NotNil(s.T(), err)
	assert.Regexp(s.T(), regexp.MustCompile("(OKTA_[A-Z_]*=, ){2}(OKTA_API_KEY=)"), err)

	os.Setenv("OKTA_API_KEY", originalOktaToken)

	err = config()
	assert.NotNil(s.T(), err)
	assert.Regexp(s.T(), regexp.MustCompile("(OKTA_[A-Z_]*=, ){2}(OKTA_API_KEY=\\[Redacted\\])"), err)
