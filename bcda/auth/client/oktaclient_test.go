...
func (s *OTestSuite) TestConfig() {
	const (
		oktaClientURL   = "OKTA_CLIENT_ORGURL"
		oktaOauthServer = "OKTA_OAUTH_SERVER_ID"
		oktaClientToken = "OKTA_CLIENT_TOKEN"
	)
	originalOktaBaseUrl := os.Getenv(oktaClientURL)
	originalOktaServerID := os.Getenv(oktaOauthServer)
	originalOktaToken := os.Getenv(oktaClientToken)

	os.Unsetenv(oktaClientURL)
	os.Unsetenv(oktaOauthServer)
	os.Unsetenv(oktaClientToken)

	err := config()
	require.NotNil(s.T(), err)
	assert.Regexp(s.T(), regexp.MustCompile("(OKTA_[A-Z_]*=, ){2}(OKTA_CLIENT_TOKEN=)"), err)

	os.Setenv(oktaClientToken, originalOktaToken)

	err = config()
	assert.NotNil(s.T(), err)
	assert.Regexp(s.T(), regexp.MustCompile("(OKTA_[A-Z_]*=, ){2}(OKTA_CLIENT_TOKEN=\\[Redacted\\])"), err)

	os.Setenv(oktaClientURL, originalOktaBaseUrl)
	os.Setenv(oktaOauthServer, originalOktaServerID)
	os.Setenv(oktaClientToken, originalOktaToken)
