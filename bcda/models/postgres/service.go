...
func (s *Service) GetNewAndExistingBeneficiaries(cmsID string, since time.Time) (newBeneficiaries, beneficiaries []*models.CCLFBeneficiary, err error) {
	...
	cclfFileNew, err := s.repository.GetLatestCCLFFile(cmsID, CCLF_8_FILE_NUM, constants.ImportComplete, cutoffTime, time.Time{})
	if err != nil {
	...
	cclfFileOld, err := s.repository.GetLatestCCLFFile(cmsID, CCLF_8_FILE_NUM, constants.ImportComplete, time.Time{}, since)
	if err != nil {
	...
	if cclfFileOld == nil {
		...
		newBeneficiaries, err = s.getBeneficaries(cclfFileNew.ID)
		if err != nil {
			return nil, nil, err
		}
	...
	benes, err := s.getBeneficaries(cclfFileNew.ID)
	if err != nil {
		return nil, nil, err
	}
	if len(benes) == 0 {
...
func (s *Service) GetBeneficiaries(cmsID string) ([]*models.CCLFBeneficiary, error) {
	var (
		cutoffTime time.Time
	)
	...
	cclfFile, err := s.repository.GetLatestCCLFFile(cmsID, CCLF_8_FILE_NUM, constants.ImportComplete, cutoffTime, time.Time{})
	if err != nil {
...
func (s *Service) getBeneficaries(cclfFileID uint) ([]*models.CCLFBeneficiary, error) {
	var (
		ignoredMBIs []string
		err         error
	)
